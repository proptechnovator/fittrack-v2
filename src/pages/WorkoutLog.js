...
function WorkoutLog() {
    ...
    const [addDisplay, setAddDisplay] = useState(false);
     ...
     useEffect(() => {
        if(currentUser) {
            ...
            async function fetchData() {
                const response = await fetch(`https://fitrack-v2.herokuapp.com/workouts?workout_user_id=${currentUser.user.user_id}&workout_date=${selectedDate}`); // route subject to change depending on server route
     ...
     async function deleteWorkout(workoutId) {
        try {
            await fetch(`https://fitrack-v2.herokuapp.com/workouts/${workoutId}`, {
                method: 'DELETE',
            });
