...
function MealLog() {
    ...
    useEffect(() => {
        if(currentUser) {
            ...
            async function fetchData() {
                const response = await fetch(`https://fitrack-v2.herokuapp.com/meals?meal_user_id=${currentUser.user.user_id}&meal_date=${selectedDate}`); // route subject to change depending on server route
    ...
    async function deleteMeal(mealId) {
        try {
            await fetch(`https://fitrack-v2.herokuapp.com/meals/${mealId}`, {
                method: 'DELETE',
            });
