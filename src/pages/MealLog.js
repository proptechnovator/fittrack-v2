...
function MealLog() {
    ...
    useEffect(() => {
        if(currentUser) {
            ...
            async function fetchData() {
                const response = await fetch(`${process.env.HEROKU_HOST_URI}/meals?meal_user_id=${currentUser.user.user_id}&meal_date=${selectedDate}`); // route subject to change depending on server route
    ...
    async function deleteMeal(mealId) {
        try {
            await fetch(`${process.env.HEROKU_HOST_URI}/meals/${mealId}`, {
                method: 'DELETE',
            });
